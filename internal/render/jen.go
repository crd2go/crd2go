...
func (jr JenRenderer) RenderDoc(req *gotype.Request, builtPlugins []plugins.Plugin, group, version string) error {
	if err := renderDocFile(req, builtPlugins, group, version); err != nil && !errors.Is(err, os.ErrExist) {
		return fmt.Errorf("failed to generate doc.go file: %w", err)
	}
	return nil
}

func (jr JenRenderer) RenderSchema(req *gotype.Request, builtPlugins []plugins.Plugin, group, version string) error {
	if err := renderSchemeFile(req, builtPlugins, group, version); err != nil && !errors.Is(err, os.ErrExist) {
...
func renderDocFile(req *gotype.Request, builtPlugins []plugins.Plugin, group, version string) error {
	f := jen.NewFile(version)
	f.HeaderComment("Code generated by crd2go. DO NOT EDIT.")
	for _, p := range builtPlugins {
		if err := p.DocAnnotate(f, group, version); err != nil {
			return fmt.Errorf("failed to run DocAnnotate on plugin %q: %w", p.Name(), err)
		}
	}
...
func renderSchemeFile(req *gotype.Request, builtPlugins []plugins.Plugin, group, version string) error {
	f := jen.NewFile(version)
	f.HeaderComment("Code generated by crd2go. DO NOT EDIT.")
	...
	for _, p := range builtPlugins {
		extra, err := p.SchemeVars(group, version)
		if err != nil {
			return fmt.Errorf("failed to run SchemeVars on plugin %q: %w", p.Name(), err)
		}
		varDefs = append(varDefs, extra...)
	}
